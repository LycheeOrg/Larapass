...
class WebAuthnAuthenticationTest extends TestCase
{
	...
	public function test_hides_from_serialization()
	{
		DB::table('web_authn_credentials')->insert([
			'id'               => Base64Url::encode('test_credential_id'),
			'name'             => 'foo',
			'user_id'          => 1,
			'type'             => 'public_key',
	...
	public function test_finds_one_by_credential_id()
	{
		...
		$this->assertInstanceOf(
			...
			$source = $model->findOneByCredentialId(Base64Url::encode('test_credential_id'))
		);
	...
	public function test_find_all_for_user_entity()
	{
		...
		DB::table('web_authn_credentials')->insert([
			...
			'type'             => 'public_key',
			'transports'       => json_encode([]),
			'attestation_type' => json_encode('none'),
			'trust_path'       => json_encode(['type' => EmptyTrustPath::class]),
			'aaguid'           => Str::uuid(),
			'public_key'       => 'public_key_bar',
			'counter'          => 0,
		...
		$this->assertSame(
			Base64Url::encode('test_credential_id'),
	...
	public function test_checks_if_credential_is_enabled_or_disabled()
	{
		...
		$credential = WebAuthnCredential::find(Base64Url::encode('test_credential_id'));

		$this->assertNull($credential);

		$credential = WebAuthnCredential::withTrashed()->find(Base64Url::encode('test_credential_id'));
